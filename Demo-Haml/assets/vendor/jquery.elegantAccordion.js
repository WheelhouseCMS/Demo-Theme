...
(function($) {
	...
	$.eAccordion = function(el, options) {
		...
		base.init = function() {
			...
			// Add formatting
			base.$items.prepend('<div class="gradient"/>').hover(function () {
			...
			}).children('.gradient, .detail').width(expandedWidth);
		...
		base.gotoPage = function(page, animate) {
			...
			if (animate !== false) {
				...
				$page.children('.title').stop(true,true).fadeOut();
				$page.children('.detail').stop(true,true).fadeIn();
				$page.children('.gradient').stop(true,true).animate(
					{bottom:0},
					base.options.animationTime
				);
				$page.siblings().children('.title').stop(true,true).fadeIn();
				$page.siblings().children('.detail').stop(true,true).fadeOut();
				$page.siblings().children('.gradient').stop(true,true).animate(
					{bottom:'-340px'},
					base.options.animationTime
				);
			} else {
				$page.width(base.options.expandedWidth).siblings().width(base.contractedWidth);
				$page.children('.title').hide();
				$page.children('.detail').show();
				$page.children('.gradient').css('bottom','0');
				$page.siblings().children('.title').show();
				$page.siblings().children('.detail').hide();
				$page.siblings().children('.gradient').css('bottom','-340px');
			}
